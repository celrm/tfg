...
int evaluate_circuit(ExpT input, CircuitT const& circuit, MatrixT & values) {
    ...
    for (LenT d = 0; d < circuit.size(); ++d) {
        for (LenT w = 0; w < circuit[0].size(); ++w) {
            p0_i = circuit[d][w][0];     // circuit[d][w][0]
            p1_i = circuit[d][w][1]; // circuit[d][w][1]
            if (d == 0) {
...
int get_function(std::vector<FunctionT> & function, CircuitT const& circuit) {
    ...
    for (ExpT input = 0; input < (1 << function.size()); ++input) { // all possible inputs 000-111
        evaluate_circuit(input, circuit, values);
        for (LenT w = 0; w < circuit[0].size(); ++w) {
            function[w][input] = values[circuit.size()-1][w]; // f[w][input] = values[D-1][w]
        }
    }
    return 0;
}